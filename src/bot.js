...
async function sendStepH(ctx) {
    await ctx.reply(
        `🌟 Шаг H (Having): Осознайте опыт\n\n` +
        `Прямо сейчас настройтесь на свое тело и окружение.\n\n` +
        `Обратите внимание:\n` +
...
async function sendStepE(ctx) {
    await ctx.reply(
        ...
        `Теперь усильте это ощущение ОК-ности:\n` +
...
async function sendStepA(ctx) {
    await ctx.reply(
        ...
        `Позвольте этому состоянию стать частью вас:\n` +
