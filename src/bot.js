...
async function sendStepH(ctx) {
    await ctx.reply(
        ...
        `❓ Как вы себя чувствуете - ОК или НЕ ОК?\n` +
...
async function handleStepInput(ctx, state) {
    ...
    if (userText.length < 10) {
        await ctx.reply('Попробуйте описать чуть подробнее - хотя бы пару предложений 😊');
        return;
    }
    ...
    try {
        ...
        // ═══════════════════════════════════════════════════════════
        // СПЕЦИАЛЬНАЯ ОБРАБОТКА ДЛЯ ШАГА H (трёхсторонняя логика)
        // ═══════════════════════════════════════════════════════════
        if (state.currentStep === 'H') {
            ...
            // Fallback если статус не найден (не должно происходить)
